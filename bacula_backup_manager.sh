...
add_job_to_bacula_config() {
    ...
    if [[ ! -f "$config_file" ]]; then
        ...
        base_config_exists=false
    elif ! grep -q "^Messages {" "$config_file" 2>/dev/null; then
        base_config_exists=false
    fi
    
    # Si no existe configuración base, crearla / If no base config, create it
    if [[ "$base_config_exists" == false ]]; then
        echo -e "${COLOR_YELLOW}⚠ Base Bacula configuration not found. Creating initial configuration...${COLOR_RESET}"
        
        # Crear configuración base mínima / Create minimal base configuration
        local director_password
        director_password=$(generate_password)
        local db_password="bacula"
        
        # Intentar obtener password de la DB si existe
        if [[ -f "$CONFIG_DIR/db_credentials.conf" ]]; then
            db_password=$(grep DB_PASSWORD "$CONFIG_DIR/db_credentials.conf" | cut -d= -f2)
        fi
...
Messages {
    Name = Standard
    mailcommand = "/usr/bin/bsmtp -h localhost -f \"bacula@localhost\" -s \"Bacula Report %t %e %c %l\" %r"
    operatorcommand = "/usr/bin/bsmtp -h localhost -f \"bacula@localhost\" -s \"Bacula Intervention Required %r\" %r"
